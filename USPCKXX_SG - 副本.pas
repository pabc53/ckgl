...
const
  ...
  cColProp1: array[0..cColQty1 - 1] of array[0..10] of string = (   //列属性
   //显示 编辑 必输 类型 对齐     颜色       宽度    标题
    ...
    ('Y', 'N', 'N', 'C', 'L', 'clWindow', '90', '商品名称', 'Y', 'POJBSUB', '3'),    // 2
    ('Y', 'N', 'N', 'C', 'L', 'clWindow', '62', '出库类型', 'Y', 'POJBSUB', '5'),    // 3
    ('Y', 'N', 'N', 'C', 'L', 'clMoneyGr', '62', '商品去向', 'Y', 'POJBSUB', '4'),    // 4
    ('Y', 'Y', 'N', 'E', 'L', 'clWindow', '60', '商品进价', 'Y', 'POJBSUB', '7'),    // 5
    ('Y', 'Y', 'N', 'E', 'L', 'clWindow', '60', '商品售价', 'Y', 'POJBSUB', '7'),    // 6
    ('Y', 'Y', 'N', 'E', 'L', 'clSkyBlue', '60', '商品数量', 'Y', 'POJBSUB', '6'),    // 7
    ('Y', 'Y', 'N', 'E', 'L', 'clWindow', '75', '商品期限', 'Y', 'POJBSUB', '10'),   // 8
    ('Y', 'Y', 'N', 'M', 'L', 'clBtnFace', '130', '备注', 'N', '', '0'),    //9
...
begin
  ...
  case ACol of
    2:
    begin
      ComboBox5.ItemIndex := AItemIndex;        //获取选择商品的编号
      with AdvStringGrid1 do
      begin
        ...
        if QueryCKX.FieldByName('进价').Value <> null then
          Cells[5, ARow] := QueryCKX.FieldByName('进价').Value;
        if QueryCKX.FieldByName('销售价').Value <> null then
          Cells[6, ARow] := QueryCKX.FieldByName('销售价').Value;
        GetQX;          //获取期限
        if ARow = 1 then                      //第一行自动填写
        begin
          AdvStringGrid1.Cells[3, ARow] := '自用';
...
begin
  case ACol of
    ...
    4:    //来源
    begin
      AEditor := edComboEdit;  //设置单元格编译类型为 下拉列表
...
begin
//  if (StringGrid1.Cells[2, cRow] = '') and (cCol > 2) then
//  begin
//    Result := False;
...
//  AdvStringGrid1.Cells[4, cRow] := cName;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox('出库数量大于库存数量！', '提示！', 64);
...
begin              //  通过供应商名称，将该供应商所属商品名称提出到ComboBoxNAME中
  ...
  begin
    ...
    for i := 0 to QueryCKX.recordcount - 1 do
    begin
      ComboBoxNAME.Items.add(QueryCKX.FieldByName('商品简称').asstring);
